...
void KeyExpansion(){
    ...
    print_str("goodbye KeyExpansion!\n");
...
int main(){
    ...
    print_str("*** AES encryption System ***\n");
    print_str("*** AES hi\n");

    
    // Hard-coded AES-128 example
    int KeySize = 128;
    Nb_k = KeySize / 32;     // Number of block of key, 計算key block數量 (Ex: AES-128 : 4) 
    Nr   = Nb_k + 6;         // Number of round(Nr),  計算AES 運算回合次數 (Ex:AES-128 : 10)
    
    // Hard-coded key: "abcdefghijklmnop"
    Key[0] = 97; Key[1] = 98; Key[2] = 99; Key[3] = 100;
    Key[4] = 101; Key[5] = 102; Key[6] = 103; Key[7] = 104;
    Key[8] = 105; Key[9] = 106; Key[10] = 107; Key[11] = 108;
    Key[12] = 109; Key[13] = 110; Key[14] = 111; Key[15] = 112;
    
    // print_str("Using AES-128\n");
    // print_str("Key: abcdefghijklmnop\n");
    
    /* Key Expansion function, 擴充鑰匙函數產生所有鑰匙 */
    KeyExpansion(); // Expansion Key - AES-128(44words/176 bytes), AES-192(52w/208 bytes), AES-256(60w/260bytes)
    
    // Hard-coded plaintext: "0123456789abcdef"
    in[0] = 48; in[1] = 49; in[2] = 50; in[3] = 51;
    in[4] = 52; in[5] = 53; in[6] = 54; in[7] = 55;
    in[8] = 56; in[9] = 57; in[10] = 97; in[11] = 98;
    in[12] = 99; in[13] = 100; in[14] = 101; in[15] = 102;
    
    // print_str("Plaintext: 0123456789abcdef\n");
    ...
    print_str("Block 0(128 bits) - plaintext(Int format) : ");
    printUnsignedCharArrayToInt(in, 16);    
    print_char('\n');
    // print Cipher(character format) in Integer Format
    print_str("Block 0(128 bits) - Cipher(Int format) : ");
    printUnsignedCharArrayToInt(out, 16);
    print_char('\n');
    
    print_str("------------------------------------------------\n");
    print_str("Encryption process complete !! \n");
